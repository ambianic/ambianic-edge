...
from ambianic.pipeline.ai.fall_detect_resnet50 import FallDetector
...
def _fall_detect_config():
    ...
    _good_tflite_model = os.path.join(
        _dir,
        'posenet_resnet50_stride16.tflite'
        )
...
def test_fall_detection_case_1():
    """Expected to detect a fall as key-points are detected by rotating second image."""
    config = _fall_detect_config()
    result = None

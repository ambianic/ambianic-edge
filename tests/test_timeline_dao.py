...
def test_get_timeline_overflow():
    data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "./timeline")
    res = timeline_dao.get_timeline(before_datetime=None, page=7, data_dir=data_dir)
    assert len(res) == 0


def test_get_timelines_no_dir():
    data_dir = "nowhere"
    res = timeline_dao.get_timeline(before_datetime=None, page=1, data_dir=data_dir)
    assert len(res) == 0
...
def test_get_timelines_before_datetime():
    ...
    res = timeline_dao.get_timeline(
        before_datetime="2020-05-10T19:05:45.577145", page=1, data_dir=data_dir
    )
    assert len(res) == 5
...
def test_get_timelines():
    ...
    for page in range(1, 6):
        res = timeline_dao.get_timeline(
            before_datetime=None, page=page, data_dir=data_dir
        )
        assert len(res) > 0
